...
# scan files
log_msg("Running Matcher fingerprinter on files in %s..." % test_folder, 
        log=options.log, silent=options.silent)

tm = time.time()
djv = MatcherTest(options.temp_folder, test_seconds)
log_msg("finished obtaining results from xmsmatch in %s" % (time.time() - tm),
        log=options.log, silent=options.silent)
